/******************************
  Program "lin.m" compiled by "Murphi Release 3.1"
...
class mu_1_val_t: public mu__long
{
 public:
  inline int operator=(int val) { return mu__long::operator=(val); };
  inline int operator=(const mu_1_val_t& val) { return mu__long::operator=((int) val); };
  mu_1_val_t (char *name, int os): mu__long(0, 1000, 10, name, os) {};
  mu_1_val_t (void): mu__long(0, 1000, 10) {};
  mu_1_val_t (int val): mu__long(0, 1000, 10, "Parameter or function result.", 0)
  {
    operator=(val);
  };
